// AfetoEmForma v5.3 — Fase 3: Onboarding Multi-Tenant
// App.jsx — Afeto em Forma · Sprint 1 + Sprint 2 + Fase 3
...
/* ═══════════════════════════════════════════════════════════════════
   AUTH CONTEXT
═══════════════════════════════════════════════════════════════════ */
const DEFAULT_TENANT_SLUG = "afeto-em-forma";
const RESERVED_PATHS = new Set(["", "reset-password", "admin", "cadastro", "login"]);

function getRouteTenantSlug() {
  if (typeof window === "undefined") return DEFAULT_TENANT_SLUG;
  const first = window.location.pathname.split("/").filter(Boolean)[0] || "";
  return RESERVED_PATHS.has(first) ? DEFAULT_TENANT_SLUG : first.toLowerCase();
}
...
function AuthProvider({ children }) {
  ...
  const fetchProfile = useCallback(async (userId) => {
    ...
    const { data } = await supabase
      .from("profiles").select("id,nome,telefone,endereco,role,tenant_id").eq("id", userId).single();
  ...
  async function signUp({ email, password, nome, telefone, endereco, tenant_id }) {
    return supabase.auth.signUp({ email, password,
      options: { data: { nome, telefone: telefone.replace(/\D/g,""), endereco, tenant_id } } });
  }
  ...
  return (
    <AuthContext.Provider value={{ session, profile, isAdmin, isPlatformAdmin, isLoggedIn, signUp, signIn, signOut, updateProfile, fetchProfile }}>
      {children}
...
/* ── CADASTRO DE NEGÓCIO (Fase 3) ── */
.onb-wrap{min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px 16px;background:var(--bg)}
.onb-card{width:100%;max-width:500px;background:var(--srf);border:1px solid var(--bdl);border-radius:var(--radd);padding:36px 32px;box-shadow:0 8px 40px var(--shd)}
.onb-logo{font-family:'Playfair Display',serif;font-size:2.2rem;font-weight:900;color:var(--pr);text-align:center;margin-bottom:4px}
.onb-logo em{color:var(--ac);font-style:normal}
.onb-sub{font-family:'Dancing Script',cursive;font-size:1.05rem;color:var(--mu);text-align:center;margin-bottom:8px}
.onb-desc{font-size:.78rem;color:var(--mum);text-align:center;margin-bottom:22px;line-height:1.7;padding:10px 8px;background:var(--srf2);border-radius:10px;border:1px solid var(--bdl)}
.onb-title{font-family:'Playfair Display',serif;font-size:1.25rem;font-weight:700;color:var(--pr);margin-bottom:18px;text-align:center}
.onb-section{font-size:.69rem;font-weight:700;color:var(--mum);letter-spacing:.08em;text-transform:uppercase;margin:20px 0 12px;display:flex;align-items:center;gap:10px}
.onb-section::before,.onb-section::after{content:'';flex:1;height:1px;background:var(--bdl)}
.onb-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.onb-field{margin-bottom:13px}
.onb-lbl{display:block;font-size:.74rem;font-weight:600;color:var(--mu);margin-bottom:5px;letter-spacing:.03em}
.onb-inp{width:100%;border:1.5px solid var(--bdl);border-radius:11px;padding:10px 13px;font-size:.87rem;font-family:'Poppins',sans-serif;color:var(--tx);background:#fff;outline:none;transition:border-color .2s,box-shadow .2s}
.onb-inp:focus{border-color:var(--pr);box-shadow:0 0 0 3px rgba(107,62,46,.08)}
.onb-inp::placeholder{color:#C0A898}
.onb-inp.error{border-color:#C03030;background:#FFF8F8}
.onb-slug-preview{font-size:.72rem;color:var(--mum);margin-top:5px;min-height:18px;padding:3px 0;transition:color .2s}
.onb-slug-preview.ok{color:var(--green);font-weight:600}
.onb-slug-preview.err{color:#C03030}
.onb-planos{display:grid;grid-template-columns:1fr 1fr;gap:11px;margin-bottom:14px}
.onb-plano{border:2px solid var(--bdl);border-radius:var(--rad);padding:14px 16px;cursor:pointer;transition:all .2s;background:var(--srf);position:relative;overflow:hidden}
.onb-plano::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--bdl);transition:background .2s}
.onb-plano.on{border-color:var(--pr);background:#FDF0E8;box-shadow:0 2px 12px rgba(107,62,46,.12)}
.onb-plano.on::before{background:linear-gradient(90deg,var(--pr),var(--sc))}
.onb-plano:hover:not(.on){border-color:var(--scl);box-shadow:0 2px 8px var(--sh)}
.onb-plano-nome{font-family:'Playfair Display',serif;font-size:1rem;font-weight:700;color:var(--pr)}
.onb-plano-preco{font-size:.8rem;color:var(--ac);font-weight:600;margin-top:3px}
.onb-plano-desc{font-size:.69rem;color:var(--mu);margin-top:6px;line-height:1.5}
.onb-btn{width:100%;padding:13px;background:linear-gradient(135deg,var(--pr),#8B5E2E);color:#fff;border:none;border-radius:12px;font-family:'Poppins',sans-serif;font-size:.95rem;font-weight:600;cursor:pointer;margin-top:10px;box-shadow:0 4px 16px rgba(107,62,46,.3);transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px}
.onb-btn:hover:not(:disabled){background:linear-gradient(135deg,var(--prd),#7A4E1A);transform:translateY(-1px);box-shadow:0 6px 20px rgba(107,62,46,.4)}
.onb-btn:disabled{opacity:.55;cursor:not-allowed}
.onb-err{background:#FFF0F0;border:1px solid #F0C0C0;color:#C03030;font-size:.78rem;padding:9px 13px;border-radius:10px;margin-bottom:13px;display:flex;align-items:flex-start;gap:6px;line-height:1.5}
.onb-ok{background:linear-gradient(135deg,#F0FBF0,#E8F8E8);border:1px solid #90C890;color:#2E6A2E;font-size:.82rem;padding:14px 16px;border-radius:12px;margin-bottom:14px;line-height:1.7}
.onb-switch{text-align:center;margin-top:18px;font-size:.8rem;color:var(--mu)}
.onb-link{color:var(--pr);font-weight:600;cursor:pointer;background:none;border:none;font-family:'Poppins',sans-serif;font-size:inherit;text-decoration:underline}
`;
...
/* ═══════════════════════════════════════════════════════════════════
   AUTH SCREEN
═══════════════════════════════════════════════════════════════════ */
function AuthScreen({ onClose, tenantId }) {
  const { signIn, signUp } = useAuth();
  const [mode,      setMode]      = useState("login");
  const [loading,   setLoading]   = useState(false);
  const [errMsg,    setErrMsg]    = useState("");
  const [okMsg,     setOkMsg]     = useState("");
  const [resetSent, setResetSent] = useState(false);   // HOTFIX: controla msg de reset
  const [fields,    setFields]    = useState({ email:"", password:"", nome:"", telefone:"", endereco:"" });
  const set = (k, v) => setFields(f => ({ ...f, [k]: v }));

  // HOTFIX: envia link de recuperação de senha
  async function handleReset() {
    if (!fields.email) { setErrMsg("Informe o e-mail para receber o link."); return; }
    setErrMsg(""); setLoading(true);
    try {
      await supabase.auth.resetPasswordForEmail(fields.email, {
        redirectTo: window.location.origin + "/reset-password",
      });
      // Mensagem genérica intencional: não revela se o e-mail existe
      setResetSent(true);
    } catch (e) {
      setErrMsg("Erro ao enviar o link. Tente novamente.");
    ...
    try {
      if (mode === "login") {
        ...
        const { error } = await signUp({ email:fields.email, password:fields.password,
          nome:fields.nome, telefone:fields.telefone, endereco:fields.endereco, tenant_id:tenantId });
        if (error) throw error;
  ...
  return (
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        {/* HOTFIX: link de recuperação de senha — visível apenas na tela de login */}
        {mode === "login" && (
          <div className="auth-switch" style={{ marginTop:10 }}>
            {resetSent ? (
              ...
              <button className="auth-link"
                onClick={handleReset}
                disabled={loading}
                style={{ color:"var(--mum)", fontWeight:400 }}>
                Esqueci minha senha
              </button>
            )}
          </div>
        ...
        <div className="auth-switch">
          {mode === "login"
            ? <>Novo por aqui? <button className="auth-link" onClick={() => { setMode("register"); setErrMsg(""); setResetSent(false); }}>Criar conta grátis</button></>
            : <>Já tem conta? <button className="auth-link" onClick={() => { setMode("login"); setErrMsg(""); setResetSent(false); }}>Entrar</button></>}
        </div>
...
/* ═══════════════════════════════════════════════════════════════════
   CADASTRO DE NEGÓCIO — Fase 3
   Formulário público que chama fn-provision-tenant via Edge Function.
   Acessível pelo botão "Cadastre seu negócio" no header/footer.
   A Edge Function usa service_role no backend e chama a RPC
   fn_provision_tenant, que fica bloqueada para anon/authenticated.
═══════════════════════════════════════════════════════════════════ */
function CadastroNegocioScreen({ onClose }) {
  const BLANK = {
    slug:"", nome:"", email:"", password:"", whatsapp:"", cidade:"", estado:"SP", plano:"free"
  };
  const [fields,   setFields]   = useState(BLANK);
  const [loading,  setLoading]  = useState(false);
  const [errMsg,   setErrMsg]   = useState("");
  const [okMsg,    setOkMsg]    = useState("");
  const [slugOk,   setSlugOk]   = useState(null); // null|true|false

  const set = (k, v) => setFields(f => ({ ...f, [k]: v }));

  // Sanitiza slug em tempo real: lowercase, só letras/números/hífen
  function handleSlugChange(raw) {
    const sanitized = raw.toLowerCase().replace(/[^a-z0-9-]/g, "-").replace(/-+/g, "-");
    set("slug", sanitized);
    setSlugOk(sanitized.length >= 3 ? true : sanitized.length > 0 ? false : null);
  }

  async function handleSubmit() {
    setErrMsg(""); setOkMsg("");
    const { slug, nome, email, password, whatsapp, cidade, estado, plano } = fields;

    // Validações client-side
    if (slug.length < 3)    { setErrMsg("A URL precisa ter pelo menos 3 caracteres."); return; }
    if (!nome.trim())        { setErrMsg("Informe o nome do negócio.");                  return; }
    if (!email.includes("@")){ setErrMsg("Informe um e-mail válido.");                  return; }
    if (password.length < 6)  { setErrMsg("A senha precisa ter pelo menos 6 caracteres."); return; }

    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/fn-provision-tenant`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) throw new Error(data.error || "Erro interno ao cadastrar negócio.");

      setOkMsg(
        `🎉 Negócio cadastrado com sucesso!\n\n` +
        `Sua URL será: afetoemforma.com/${data.slug || slug}\n\n` +
        `Use o e-mail ${email} e a senha cadastrada para entrar no painel administrativo.`
      );
      setFields(BLANK);
    } catch (e) {
      // fn_provision_tenant usa RAISE EXCEPTION com mensagens amigáveis
      const msg = e.message ?? "";
      if (msg.includes("Slug já está em uso") || msg.includes("Slug ja esta em uso"))
        setErrMsg(`A URL "${slug}" já está sendo usada. Tente outra.`);
      else if (msg.includes("E-mail já cadastrado") || msg.includes("E-mail ja cadastrado"))
        setErrMsg("Este e-mail já tem um negócio cadastrado.");
      else if (msg.includes("Slug muito curto"))
        setErrMsg("A URL precisa ter pelo menos 3 caracteres.");
      else if (msg.includes("Senha invalida") || msg.includes("Password") || msg.includes("password"))
        setErrMsg("A senha precisa ter pelo menos 6 caracteres.");
      else
        setErrMsg("Erro ao cadastrar. Tente novamente ou entre em contato.");
      console.error("[AeF] provision_tenant:", e);
    } finally {
      setLoading(false);
    }
  }
  ...
  const PLANOS = [
    { id:"free", nome:"Gratuito",  preco:"R$ 0/mês",  desc:"Até 4 fornadas/mês · 20 produtos" },
    { id:"pro",  nome:"Pro",       preco:"R$ 97/mês", desc:"Fornadas ilimitadas · Relatórios avançados" },
  ];

  return (
    <div className="onb-wrap">
      <div className="onb-card">
        <div className="onb-logo">Afeto em <em>Forma</em></div>
        <div className="onb-sub">Cuidado em cada pedaço.</div>
        <div className="onb-desc">
          Gerencie pedidos, fornadas e relatórios do seu negócio artesanal.<br/>
          Configure em minutos. Sem mensalidade para começar.
        </div>

        {okMsg ? (
          <div>
            <div className="onb-ok" style={{ whiteSpace:"pre-line" }}>{okMsg}</div>
            <button className="onb-btn" onClick={() => { setOkMsg(""); onClose?.(); }}>
              ← Voltar ao cardápio
            </button>
          </div>
        ) : (
          <>
            <div className="onb-title">Cadastre seu negócio</div>

            {errMsg && <div className="onb-err">⚠️ {errMsg}</div>}

            {/* URL do negócio */}
            <div className="onb-section">Sua URL pública</div>
            <div className="onb-field">
              <label className="onb-lbl">Identificador (slug) *</label>
              <input className={`onb-inp${slugOk === false ? " error" : ""}`}
                placeholder="padaria-silva"
                value={fields.slug}
                onChange={e => handleSlugChange(e.target.value)} />
              <div className={`onb-slug-preview ${slugOk === true ? "ok" : slugOk === false ? "err" : ""}`}>
                {fields.slug.length === 0
                  ? "ex: afetoemforma.com/padaria-silva"
                  : slugOk === false
                  ? "⚠️ Mínimo 3 caracteres"
                  : `✓ afetoemforma.com/${fields.slug}`}
              </div>
            </div>

            {/* Dados do negócio */}
            <div className="onb-section">Dados do negócio</div>
            <div className="onb-field">
              <label className="onb-lbl">Nome do negócio *</label>
              <input className="onb-inp" placeholder="Padaria Silva"
                value={fields.nome} onChange={e => set("nome", e.target.value)} />
            </div>
            <div className="onb-grid">
              <div className="onb-field">
                <label className="onb-lbl">WhatsApp (com DDD)</label>
                <input className="onb-inp" placeholder="(12) 99999-9999"
                  value={fields.whatsapp} onChange={e => set("whatsapp", e.target.value)} />
              </div>
              <div className="onb-field">
                <label className="onb-lbl">Cidade</label>
                <input className="onb-inp" placeholder="São Sebastião"
                  value={fields.cidade} onChange={e => set("cidade", e.target.value)} />
              </div>
            </div>
            <div className="onb-field">
              <label className="onb-lbl">Estado</label>
              <input className="onb-inp" placeholder="SP" maxLength={2}
                value={fields.estado} onChange={e => set("estado", e.target.value.toUpperCase().replace(/[^A-Z]/g, ""))} />
            </div>

            {/* Conta de acesso */}
            <div className="onb-section">Conta de acesso</div>
            <div className="onb-field">
              <label className="onb-lbl">E-mail do administrador *</label>
              <input className="onb-inp" type="email" placeholder="seu@email.com"
                value={fields.email} onChange={e => set("email", e.target.value)}
                onKeyDown={e => e.key === "Enter" && handleSubmit()} />
            </div>
            <div className="onb-field">
              <label className="onb-lbl">Senha do administrador *</label>
              <input className="onb-inp" type="password" placeholder="Mínimo 6 caracteres"
                value={fields.password} onChange={e => set("password", e.target.value)}
                onKeyDown={e => e.key === "Enter" && handleSubmit()} />
            </div>

            {/* Plano */}
            <div className="onb-section">Escolha seu plano</div>
            <div className="onb-planos">
              {PLANOS.map(p => (
                <div key={p.id}
                  className={`onb-plano ${fields.plano === p.id ? "on" : ""}`}
                  onClick={() => set("plano", p.id)}>
                  <div className="onb-plano-nome">{p.nome}</div>
                  <div className="onb-plano-preco">{p.preco}</div>
                  <div className="onb-plano-desc">{p.desc}</div>
                </div>
              ))}
            </div>

            <button className="onb-btn" onClick={handleSubmit} disabled={loading}>
              {loading
                ? <><span className="spin">⏳</span> Cadastrando...</>
                : "🚀 Criar meu negócio grátis"}
            </button>

            <div className="onb-switch">
              Já tem conta?{" "}
              <button className="onb-link" onClick={onClose}>← Voltar ao cardápio</button>
            </div>
          </>
        )}
      </div>
...
═══════════════════════════════════════════════════════════════════ */
// ─── HOTFIX: modal de edição de perfil ──────────────────────
// Arquivo: App.jsx (v5.3+, ~1.950 linhas)
// SQL necessário: não (updateProfile já existe no AuthProvider)
// Três patches independentes — aplicar na ordem abaixo.

function AfetoEmFormaApp() {
  const { session, profile, isAdmin, isPlatformAdmin, isLoggedIn, signOut, updateProfile } = useAuth();
  const routeTenantSlug = getRouteTenantSlug();

  /* ─── navegação ─── */
  const [showAuth,       setShowAuth]       = useState(false);
  const [showCadastro,   setShowCadastro]   = useState(false); // Fase 3: formulário de cadastro
  ...
  useEffect(() => {
    ...
    async function resolveTenant() {
      ...
      const { data, error } = await supabase
        .from("tenant_public")
        .select("id,slug,nome,cidade,estado,whatsapp_num,cor_primaria,cor_acento,status")
        .eq("slug", routeTenantSlug)
        .maybeSingle();

      if (!alive) return;
      if (error) {
        console.error("[AeF] resolveTenant:", error.message);
        setTenantErr("NÃ£o foi possÃ­vel carregar este negÃ³cio.");
        setActiveTenant(null);
      } else if (!data) {
        setTenantErr(`NegÃ³cio "${routeTenantSlug}" nÃ£o encontrado.`);
        setActiveTenant(null);
      } else {
  ...
  const fetchFornadas = useCallback(async () => {
    if (!activeTenant?.id) { setFornadas([]); setLoading(false); return; }
    setLoading(true);
  ...
  const fetchProdutos = useCallback(async ({ apenasAtivos = true } = {}) => {
    if (!activeTenant?.id) { setProdutos([]); setLoadProd(false); return; }
    setLoadProd(true);
    // ── FASE 2 / RLS: isolamento por tenant já garantido pelo JWT via get_tenant_id().
  ...
  const fetchPlatformTenants = useCallback(async () => {
    ...
    setLoadPlatform(true);
    const { data, error } = await supabase
      .from("tenants")
      .select("id,slug,nome,email_admin,plano,status,cidade,estado,created_at,trial_ends_at,max_fornadas_mes,max_produtos,max_admins")
      .order("created_at", { ascending: false });

    if (!error && data) setPlatformTenants(data);
    else if (error) console.error("[AeF] fetchPlatformTenants:", error.message);
    setLoadPlatform(false);
  }, [isPlatformAdmin]);
  ...
  async function saveProduto() {
    ...
    const payload = {
      ...
      // FIX 403: tenant_id obrigatório pelo WITH CHECK da policy produtos_manage_tenant_admin.
      // No INSERT o campo não existe ainda no banco — precisa vir do profile do admin logado.
      // No UPDATE o banco já tem o valor; incluímos por consistência mas é redundante.
  ...
  /* ══════════════════════════════════════════════════════════
     RENDER — tela de auth
  ══════════════════════════════════════════════════════════ */
  if (showAuth)     return <AuthScreen          onClose={() => setShowAuth(false)} tenantId={activeTenant?.id} />;
    ...
    <>
      <div style={{ minHeight:"100vh" }}>
        ...
        <footer>
          ...
          {/* Fase 3: CTA para novos negócios no rodapé */}
          {!isLoggedIn && (
            <button
              onClick={() => setShowCadastro(true)}
              style={{
                marginTop:16, background:"rgba(255,255,255,.08)",
                border:"1px solid rgba(227,183,120,.3)", color:"#E3B778",
                borderRadius:20, padding:"8px 20px", fontFamily:"'Poppins',sans-serif",
...
{/* ══════════════════ MODAL EDITAR PERFIL ══════════════════ */}
      {/* HOTFIX: modal de edição de nome, telefone e endereço do usuário logado */}
      {showEditPerfil && (
        <div className="mbg" onClick={e => e.target === e.currentTarget && setShowEditPerfil(false)}>
          <div className="mbox">
            <button className="mclose" onClick={() => setShowEditPerfil(false)}>✕</button>
            <div className="mhandle" />
            {perfilOk ? (
              <div className="succ">
                <div className="succ-ico">✅</div>
                <div className="succ-t">Perfil atualizado!</div>
                <div className="succ-s">Suas informações foram salvas com sucesso.</div>
                <button className="msub" style={{ marginTop:18 }} onClick={() => setShowEditPerfil(false)}>Fechar</button>
              </div>
            ) : (
              <>
                <div className="mtitle">Editar Perfil</div>
                {/* E-mail: somente leitura */}
                <div className="mfield">
                  <label className="mlbl">E-mail (não editável)</label>
                  <input className="minput" disabled value={session?.user?.email || ""}
                    style={{ opacity:.6, cursor:"not-allowed", background:"var(--srf2)" }} />
                </div>
                <div className="mfield">
                  <label className="mlbl">Nome *</label>
                  <input className="minput" placeholder="Seu nome completo"
                ...
                <div className="mfield">
                  <label className="mlbl">Endereço *</label>
                  <input className="minput" placeholder="Rua, número, bairro"
                    value={perfilForm.endereco}
                    onChange={e => setPerfilForm(f => ({ ...f, endereco: e.target.value }))} />
                </div>
                {perfilErr && <div className="merr">⚠️ {perfilErr}</div>}
                <button className="msub" disabled={perfilSaving} onClick={async () => {
                  if (!perfilForm.nome.trim())     { setPerfilErr("Nome é obrigatório.");      return; }
                  if (!perfilForm.telefone.trim()) { setPerfilErr("WhatsApp é obrigatório."); return; }
                  if (!perfilForm.endereco.trim()) { setPerfilErr("Endereço é obrigatório."); return; }
                  setPerfilErr(""); setPerfilSaving(true);
                  const { error } = await updateProfile({
                    nome:     perfilForm.nome.trim(),
                    telefone: perfilForm.telefone.trim(),
                    endereco: perfilForm.endereco.trim(),
                  });
                  setPerfilSaving(false);
                  if (error) setPerfilErr("Erro ao salvar. Tente novamente.");
                  else setPerfilOk(true);
                }}>
                  {perfilSaving ? <><span className="spin">⏳</span> Salvando...</> : "💾 Salvar Perfil"}
                </button>
              </>
            )}
          </div>
      ...
      {adminOpen && isAdminForCurrentTenant && (
        <div className="adm-ov">
          <div className="adm-hdr">
            ...
            <span className="adm-htitle">⚙️ {isPlatformAdmin ? "Painel da Plataforma" : "Painel da Artesã"} — {activeTenant?.nome || "Afeto em Forma"}</span>
          </div>
          <div className="adm-body">
            ...
            <div className="atabs">
              {[
                ...(isPlatformAdmin ? [["plataforma","🌐 Plataforma"]] : []),
                ["fornadas","🔥 Fornadas"],
                ["catalogo","📦 Catálogo"],
                ["pedidos","📋 Pedidos"],
                ["grafico","📊 Gráfico"],
            ...
            {/* ── TAB PLATAFORMA ── */}
            {adminTab === "plataforma" && isPlatformAdmin && (
              <div>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14, gap:12, flexWrap:"wrap" }}>
                  <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.15rem", color:"var(--pr)", fontWeight:700 }}>
                    🌐 Tenants da plataforma
                  </div>
                  <button className="adm-refresh" onClick={fetchPlatformTenants} disabled={loadPlatform}>
                    {loadPlatform ? <><span className="spin">⏳</span> Carregando...</> : "↻ Atualizar"}
                  </button>
                </div>
                <div className="kpi-grid" style={{ marginBottom:16 }}>
                  <div className="kpi"><div className="kpi-val">{platformTenants.length}</div><div className="kpi-lbl">Tenants</div></div>
                  <div className="kpi"><div className="kpi-val">{platformTenants.filter(t => t.status === "active").length}</div><div className="kpi-lbl">Ativos</div></div>
                  <div className="kpi"><div className="kpi-val">{platformTenants.filter(t => t.status === "trial").length}</div><div className="kpi-lbl">Trial</div></div>
                  <div className="kpi"><div className="kpi-val">{platformTenants.filter(t => t.plano === "pro").length}</div><div className="kpi-lbl">Plano Pro</div></div>
                </div>
                {loadPlatform && <div style={{ textAlign:"center", padding:"24px", color:"var(--mu)" }}><span className="spin">🔥</span> Buscando tenants...</div>}
                {!loadPlatform && platformTenants.length === 0 && <div style={{ textAlign:"center", color:"var(--mu)", padding:"32px" }}>Nenhum tenant encontrado.</div>}
                <div style={{ display:"grid", gap:10 }}>
                  {platformTenants.map(t => (
                    <div key={t.id} className="porder" style={{ alignItems:"flex-start" }}>
                      <span className="porder-e">🏪</span>
                      <div className="porder-info">
                        <div className="porder-nm">
                          {t.nome}
                          <span style={{ fontWeight:400, color:"var(--mu)", marginLeft:6 }}>/{t.slug}</span>
                        </div>
                        <div className="porder-meta">
                          {t.email_admin} · {[t.cidade, t.estado].filter(Boolean).join(", ") || "sem cidade"} · criado em {new Date(t.created_at).toLocaleDateString("pt-BR")}
                        </div>
                        <div className="porder-meta">
                          limites: {t.max_fornadas_mes} fornadas/mês · {t.max_produtos} produtos · {t.max_admins} admin(s)
                        </div>
                      </div>
                      <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6, flexShrink:0 }}>
                        <span className={`badge ${t.status === "active" ? "bc" : "bp"}`}>{t.status}</span>
                        <span className="pcat-price" style={{ fontSize:".9rem" }}>{t.plano}</span>
                      </div>
                    </div>
                  ))}
                </div>
