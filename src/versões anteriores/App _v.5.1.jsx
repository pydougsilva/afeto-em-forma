// AfetoEmForma v5.1 — Merge: Catálogo Dinâmico + Relatórios de Vendas
...
function AfetoEmFormaApp() {
  ...
  async function handleCheckout() {
    ...
    // ── Validação 24h de antecedência para bolos ──────────────────────
    
    if (prod.tipo === "bolo" && selFornada && typeof selFornada === "string") {
      const hoje     = new Date();
      hoje.setHours(0, 0, 0, 0);
      const amanha   = new Date(hoje);
      amanha.setDate(amanha.getDate() + 1);
      const escolhida = new Date(selFornada + "T00:00:00");
      if (escolhida < amanha) {
        setErr(`Pedidos de ${prod.n} precisam de no mínimo 24 horas de antecedência.`);
      return;
  }
}
