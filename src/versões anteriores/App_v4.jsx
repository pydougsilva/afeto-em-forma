// DosAnjos_v4.jsx — Autenticação completa com Supabase Auth
...
.hdr-rbtn{background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.15);color:rgba(255,255,255,.7);font-size:.7rem;padding:6px 12px;border-radius:8px;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s}
.hdr-rbtn:hover{background:rgba(255,255,255,.2);color:#fff}
.hdr-rbtn.danger{border-color:rgba(231,111,81,.4);color:rgba(231,111,81,.85)}
.hdr-rbtn.danger:hover{background:rgba(231,111,81,.18);color:#E76F51}
.hdr-user{display:flex;align-items:center;gap:6px;background:rgba(255,255,255,.1);border:1px solid rgba(196,154,108,.3);border-radius:20px;padding:5px 13px;font-size:.71rem;color:#E8D5C0}
...
.cta-primary:hover{background:linear-gradient(135deg,var(--prd),#8B3030);transform:translateY(-1px)}
...
.minput{width:100%;border:1.5px solid var(--bdl);border-radius:11px;padding:10px 13px;font-size:.87rem;font-family:'DM Sans',sans-serif;color:var(--tx);background:#fff;outline:none;transition:border-color .2s;appearance:none}
...
.auth-sub{font-family:'Playfair Display',serif;font-style:italic;font-size:.9rem;color:var(--mu);text-align:center;margin-bottom:24px}
...
.auth-inp{width:100%;border:1.5px solid var(--bdl);border-radius:11px;padding:10px 13px;font-size:.87rem;font-family:'DM Sans',sans-serif;color:var(--tx);background:#fff;outline:none;transition:border-color .2s}
.auth-inp:focus{border-color:var(--pr)}
.auth-inp::placeholder{color:#C0A898}
.auth-btn{width:100%;padding:13px;background:linear-gradient(135deg,var(--pr),#A04040);color:#fff;border:none;border-radius:12px;font-family:'DM Sans',sans-serif;font-size:.95rem;font-weight:600;cursor:pointer;margin-top:5px;box-shadow:0 4px 16px rgba(107,43,43,.3);transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px}
.auth-btn:hover:not(:disabled){background:linear-gradient(135deg,var(--prd),#8B3030);transform:translateY(-1px)}
.auth-btn:disabled{opacity:.55;cursor:not-allowed}
.auth-switch{text-align:center;margin-top:18px;font-size:.8rem;color:var(--mu)}
.auth-link{color:var(--pr);font-weight:600;cursor:pointer;background:none;border:none;font-family:'DM Sans',sans-serif;font-size:inherit;text-decoration:underline}
  ...
  return (
    <div className="auth-wrap">
      <div className="auth-card">
        <div className="auth-logo">Dos<em>Anjos</em></div>
        <div className="auth-sub">Forno a Lenha · São Sebastião</div>
...
function DosAnjosApp() {
  ...
  function handleWaitlist(prod) {
    ...
    const msg  = `Olá DosAnjos! 👋 Vi que todas as fornadas de *${prod.n}* estão lotadas. Gostaria de entrar na *Lista de Espera*. Meu nome é *${nome}*.`;
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      const msg = prod.tipo === "bolo"
        ? `Olá DosAnjos! 🎂 Encomendar *${qty}x ${prod.n}* para *${dtLabel}*.\n\nNome: *${nome}*\nWhatsApp: *${telefone}*\nEndereço: *${endereco}*`
        : prod.tipo === "pao"
        ? `Olá DosAnjos! 🍞 Garantir vaga para *${qty}x ${prod.n}* — fornada de *${dtLabel}*.\n\nNome: *${nome}*\nWhatsApp: *${telefone}*\nEndereço: *${endereco}*`
        : `Olá DosAnjos! 🍪 Reservar *${qty} pacote(s) de ${prod.n}* — fornada de *${dtLabel}*.\n\nNome: *${nome}*\nWhatsApp: *${telefone}*\nEndereço: *${endereco}*`;
