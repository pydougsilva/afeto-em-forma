...
/* ═══════════════════════════════════════════════════════════════════
   CADASTRO DE NEGÓCIO — Fase 3
   Formulário público que chama fn_provision_tenant via RPC.
   Acessível pelo botão "Cadastre seu negócio" no header/footer.
   fn_provision_tenant tem REVOKE para anon/authenticated —
   a chamada usa a anon_key mas a função roda como SECURITY DEFINER
   no banco (postgres), portanto tem acesso total às tabelas.
═══════════════════════════════════════════════════════════════════ */
function CadastroNegocioScreen({ onClose }) {
  const BLANK = {
    slug:"", nome:"", email:"", whatsapp:"", cidade:"", plano:"free"
  };
  ...
  async function handleSubmit() {
    setErrMsg(""); setOkMsg("");
    const { slug, nome, email, whatsapp, cidade, plano } = fields;
    ...
    setLoading(true);
    try {
      // fn_provision_tenant: SECURITY DEFINER, corre como postgres
      // Erros de duplicata são surfaceados via RAISE EXCEPTION → message
      const { data, error } = await supabase.rpc("fn_provision_tenant", {
        p_slug:        slug,
        p_nome:        nome.trim(),
        p_email_admin: email.trim().toLowerCase(),
      ...
      setOkMsg(
        ...
        `Sua URL será: afetoemforma.com/${slug}\n\n` +
        `Em breve você receberá um e-mail em ${email} com as instruções de acesso ao painel. ` +
        `Enquanto isso, nossa equipe está configurando seu espaço.`
      );
      setFields(BLANK);
    } catch (e) {
...
function AfetoEmFormaApp() {
  const { session, profile, isAdmin, isLoggedIn, signOut, updateProfile } = useAuth();
